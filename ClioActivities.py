...
class ClioActivities(ClioAPIHelper):
    def __init__(self, client_id, client_secret, attorney, start_date, end_date):
        ...
        CLIO_ACTIVITY_URL = (
            f"{self.CLIO_API_URL}activities?"
        ...
        response = self.make_authenticated_clio_request(
        ...
        if response.status_code == 200:
            ...
            print(f"Found {len(self.activities_data['data'])} activities for {self.month_name}")
        else:
    ...
    def print_activity(self, activity: dict):
        ...
        print(f" - Date: {dd_date}, Activity: {category}, Duration: {activity['quantity_in_hours']:.2f}, Description: {activity['note']}, User: ID: {activity['user']['id']} Name: {activity['user']['name']}")

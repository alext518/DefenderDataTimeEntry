...
class MyCaseAPIHelper:
    def __init__(self, client_id, client_secret, attorney: Attorney, redirect_uri: str = "", state: str = "state", response_type: str = "code", redirect_on_decline: bool = False):
        ...
        self.redirect_on_decline = redirect_on_decline
        self.token_store: dict = self.read_tokens_from_file()
        self.grant_type = "authorization_code"
        self.MYCASE_BASE_URL = "https://auth.mycase.com"
        self.MYCASE_AUTH_URL = f"{self.MYCASE_BASE_URL}/login_sessions/new"
        self.MYCASE_TOKEN_URL = f"{self.MYCASE_BASE_URL}/tokens"
        self.MYCASE_API_URL = "https://external-integrations.mycase.com/v1"
        self.MAX_PAGE_SIZE = 1000


    def get_new_token(self):
        auth_url = (
                f"https://{self.MYCASE_AUTH_URL}?"
    ...
    def refresh_mycase_token(self):
        ...
        response = requests.post(self.MYCASE_TOKEN_URL, data=refresh_data)
        response.raise_for_status()
    ...
    def make_authenticated_mycase_request(self, url, method='GET', **kwargs):
        if self.is_token_expired():
            self.refresh_mycase_token()
